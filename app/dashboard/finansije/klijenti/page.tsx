...
function clientToForm(c: Client): ClientFormData {
  return {
    name: c.name, pib: c.pib ?? "", maticni_broj: c.maticni_broj ?? "",
    ziro_racun: c.ziro_racun ?? "", address: c.address ?? "",
...
function Toast({ message, onDone }: { message: string; onDone: () => void }) {
  ...
  return (
    <div style={{ position: "fixed", bottom: 90, left: "50%", transform: "translateX(-50%)", background: "#111418", color: "#fff", borderRadius: 10, padding: "11px 20px", fontSize: 14, fontWeight: 500, boxShadow: "0 8px 28px rgba(16,24,40,.22)", zIndex: 300, whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 8 }}>
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", background: "#fff", borderRadius: 18, padding: "32px 28px 24px", width: 380, zIndex: 201, boxShadow: "0 20px 60px rgba(16,24,40,.18)", textAlign: "center" }}>
        ...
        <div style={{ fontSize: 17, fontWeight: 700, color: "#111418", marginBottom: 8 }}>Obriši klijenta?</div>
...
interface SlideOverProps { open: boolean; editing: Client | null; onClose: () => void; onSaved: (name: string) => void; }

function ClientSlideOver({ open, editing, onClose, onSaved }: SlideOverProps) {
  const firstRef = useRef<HTMLInputElement>(null);
  const qc = useQueryClient();

  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } =
    useForm<ClientFormData>({ defaultValues: EMPTY_CLIENT_FORM });

  useEffect(() => {
    if (open) {
      reset(editing ? clientToForm(editing) : EMPTY_CLIENT_FORM);
  ...
  const saveMut = useMutation({
    mutationFn: (data: ClientFormData) => {
      const payload = {
        ...
        ziro_racun:   data.ziro_racun   || null,
        address:      data.address      || null,
        email:        data.email        || null,
        phone:        data.phone        || null,
      };
      return editing
        ? api.put(`${BASE}/${editing.id}`, payload).then((r) => r.data as Client)
        : api.post(BASE, payload).then((r) => r.data as Client);
    },
    onSuccess: (c: Client) => {
  ...
  if (!open) return null;
  const fo = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => (e.target.style.borderColor = "var(--green)");
  const fb = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => (e.target.style.borderColor = "var(--border)");
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: 480, background: "#fff", zIndex: 101, display: "flex", flexDirection: "column", boxShadow: "-8px 0 40px rgba(16,24,40,.14)", animation: "slideInRight .25s cubic-bezier(.32,.72,.27,1)" }}>
        <div style={{ padding: "22px 24px 18px", borderBottom: "1px solid var(--border-soft)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: "var(--green-soft)", color: "var(--green)", display: "grid", placeItems: "center" }}>
              <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="7" width="20" height="14" rx="2" /><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16" /></svg>
            </div>
            <div>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#111418" }}>{editing ? "Izmijeni klijenta" : "Nov klijent"}</div>
              <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 1 }}>{editing ? editing.name : "Dodaj novog klijenta"}</div>
            </div>
          </div>
        ...
        <form id="client-form" onSubmit={handleSubmit((d) => saveMut.mutate(d))} style={{ flex: 1, overflowY: "auto", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            <label style={labelStyle}>Naziv firme</label>
            <input type="text" placeholder="npr. FIRMA d.o.o."
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              ...
              <input type="text" placeholder="123456789" {...register("pib")} style={{ ...inputStyle, fontFamily: "var(--font-geist-mono), monospace" }} onFocus={fo} onBlur={fb} />
            </div>
            <div>
              <label style={labelStyle}>Matični broj <span style={{ fontWeight: 400, color: "var(--muted)" }}>(opciono)</span></label>
              <input type="text" placeholder="12345678" {...register("maticni_broj")} style={{ ...inputStyle, fontFamily: "var(--font-geist-mono), monospace" }} onFocus={fo} onBlur={fb} />
            </div>
          </div>

          <div>
            <label style={labelStyle}>Žiro račun <span style={{ fontWeight: 400, color: "var(--muted)" }}>(opciono)</span></label>
            <input type="text" placeholder="265-0000000000-00" {...register("ziro_racun")} style={{ ...inputStyle, fontFamily: "var(--font-geist-mono), monospace" }} onFocus={fo} onBlur={fb} />
          </div>

          <div>
            <label style={labelStyle}>Adresa <span style={{ fontWeight: 400, color: "var(--muted)" }}>(opciono)</span></label>
            <input type="text" placeholder="Ulica i broj, grad" {...register("address")} style={inputStyle} onFocus={fo} onBlur={fb} />
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              <label style={labelStyle}>Email <span style={{ fontWeight: 400, color: "var(--muted)" }}>(opciono)</span></label>
              <input type="email" placeholder="firma@example.com" {...register("email")} style={inputStyle} onFocus={fo} onBlur={fb} />
            </div>
            <div>
              <label style={labelStyle}>Telefon <span style={{ fontWeight: 400, color: "var(--muted)" }}>(opciono)</span></label>
              <input type="text" placeholder="+381 11 123 4567" {...register("phone")} style={inputStyle} onFocus={fo} onBlur={fb} />
            </div>
          </div>
        ...
        <div style={{ padding: "14px 24px 20px", borderTop: "1px solid var(--border-soft)", display: "flex", gap: 10, justifyContent: "flex-end" }}>
          ...
          <button type="submit" form="client-form" disabled={isSubmitting || saveMut.isPending} style={{ padding: "9px 22px", border: "none", borderRadius: 9, background: isSubmitting || saveMut.isPending ? "#4ade80" : "var(--green)", color: "#fff", fontSize: 14, fontWeight: 600, cursor: isSubmitting || saveMut.isPending ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
            {isSubmitting || saveMut.isPending ? "Čuvanje..." : editing ? "Sačuvaj izmene" : "Dodaj klijenta"}
          </button>
        </div>
      </div>
    </>
...
export default function KlijentiPage() {
  ...
  const { data: paginatedData, isLoading } = useQuery<PaginatedClients>({
    queryKey: ["clients", TENANT, debouncedSearch, page],
    queryFn: ({ signal }) =>
      api.get(BASE, { signal, params: { search: debouncedSearch, page, per_page: 25 } }).then((r) => r.data),
    staleTime: 30_000,
  });
    ...
    <PageShell navId="fin">
      <div style={{ padding: "var(--hero-padding, 8px 32px 6px)", borderBottom: "1px solid var(--border-soft)", display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 16 }}>
        <div>
          ...
          <h1 style={{ fontSize: "var(--hero-h1, 28px)", fontWeight: 700, margin: "4px 0", letterSpacing: "-0.02em", color: "#111418" }}>Klijenti</h1>
          <p style={{ margin: "0 0 8px", fontSize: 15, color: "var(--muted)" }}>Evidencija i upravljanje klijentima firme.</p>
        </div>
        <button
      ...
      <div style={{ padding: "24px 32px 110px" }}>
        <div style={{ marginBottom: 16, position: "relative", maxWidth: 400 }}>
          ...
          <input type="text" placeholder="Pretraži po nazivu ili PIB-u..." value={search}
        ...
        <div style={{ background: "#fff", border: "1px solid var(--border)", borderRadius: 16, overflow: "hidden", boxShadow: "var(--shadow-card)" }}>
          {isLoading ? (
            <div style={{ padding: "56px 0", textAlign: "center", color: "var(--muted)", fontSize: 14 }}>Učitavanje klijenata...</div>
          ) : clients.length === 0 ? (
            <div style={{ padding: "72px 0", textAlign: "center", color: "var(--muted)" }}>
              ...
              <p style={{ margin: 0, fontSize: 15, fontWeight: 500 }}>{debouncedSearch ? `Nema rezultata za "${debouncedSearch}"` : "Nema klijenata"}</p>
              {!debouncedSearch && <p style={{ margin: "6px 0 0", fontSize: 13.5 }}>Dodajte prvog klijenta klikom na dugme iznad.</p>}
            </div>
          ) : (
                  ...
                  {sortedClients.map((c) => (
                    ...
                    >
                      ...
                      <td style={{ ...tdStyle, fontFamily: "var(--font-geist-mono), monospace", fontSize: 13 }}>
                        {c.pib ?? <span style={{ color: "var(--muted-2)" }}>—</span>}
                      </td>
                      <td style={{ ...tdStyle, fontFamily: "var(--font-geist-mono), monospace", fontSize: 13, maxWidth: 180 }}>
                        {c.ziro_racun ? <span style={{ display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={c.ziro_racun}>{c.ziro_racun}</span> : <span style={{ color: "var(--muted-2)" }}>—</span>}
                      </td>
                      <td style={{ ...tdStyle, color: "var(--muted)", maxWidth: 180 }}>
                        {c.email ? <span style={{ display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={c.email}>{c.email}</span> : <span style={{ color: "var(--muted-2)" }}>—</span>}
                      </td>
                      <td style={tdStyle}>{c.phone ?? <span style={{ color: "var(--muted-2)" }}>—</span>}</td>
                      <td style={{ ...tdStyle, textAlign: "center" }}>
                        <div style={{ display: "inline-flex", gap: 6 }}>
                          <button onClick={() => { setEditing(c); setSlideOpen(true); }} title="Izmijeni"
                          ...
                          <button onClick={() => setDeleteTarget(c)} title="Obriši"
                            style={{ width: 32, height: 32, borderRadius: 8, border: "1px solid var(--border)", background: "#fff", cursor: "pointer", display: "grid", placeItems: "center", color: "var(--muted)", transition: "all .12s" }}
        ...
        {total > 0 && (
          <div style={{ marginTop: 12, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: 13, color: "var(--muted)", paddingLeft: 4 }}>
              {total} {total === 1 ? "klijent" : total < 5 ? "klijenta" : "klijenata"}
            </span>
            {lastPage > 1 && (
      ...
      <ClientSlideOver open={slideOpen} editing={editing}
        onClose={() => setSlideOpen(false)}
        onSaved={(name) => setToast(editing ? `"${name}" uspješno ažuriran.` : `Klijent "${name}" dodan.`)}
      />
